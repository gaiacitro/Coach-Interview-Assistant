...
def cv_performance_evaluation(cv_data_dict):
    ...
    # calculating the "error seconds" per minute of video
    ...
    # Creating different times for different metrics to avoid double-counting (e.g., hands on the face implies hands above the chin)
    ...
    # Put weights on the values per minute based on the severity of the gesture
    hand_gravity_raw = (0.6 * hand_general_pm) + (0.4 * big_gestures_pm) + (1.3 * touching_face_pm) 


    # Create a dictionary to hold the evaluated report with calculated values for each metric
    evaluated_report = {}
    
    # Limiting the final gravity score to 100.0 as the maximum
    evaluated_report["head_total"] = min(head_total_raw, 100.0)
    evaluated_report["hand_gravity"] = min(hand_gravity_raw, 100.0)
    
    # Keeping the evaluation of individual "dots" unchanged 
...
def speech_performance_evaluation(speech_data_dict):
    # Calculate the duration avoiding division by zero
    ...
    # Evaluate each speech metric and store the results in the evaluated_report dictionary
    ...
    # Calulate the total speech gravity score 
...
def calculate_perfection_score(gravity_value, thresholds):
    ...
    # LOWER YELLOW ZONE (Too little movement, slightly rigid) -> Score 33 to 66
    ...
    # LOWER RED ZONE (Completely frozen/statue) -> Score 0 to 33
    ...
    # UPPER YELLOW ZONE (Too much movement, slightly anxious) -> Score 33 to 66
    ...
    # UPPER RED ZONE (Excessive movement, very anxious) -> Score 0 to 33
